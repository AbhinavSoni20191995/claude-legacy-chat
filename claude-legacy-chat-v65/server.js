...
// For users who upload large context files. Chunks the file and summarizes each chunk.
app.post('/api/compress-file', async (req, res) => {
  const { content, apiKey, filename } = req.body;
  const key = apiKey || process.env.ANTHROPIC_API_KEY;
  if (!key) return res.status(401).json({ error: 'No API key provided.' });
  if (!content || typeof content !== 'string') return res.status(400).json({ error: 'No content provided.' });

  try {
    const CHUNK_SIZE = 30000; // chars (~7500 tokens)
    const chunks = [];
    for (let i = 0; i < content.length; i += CHUNK_SIZE) {
      chunks.push(content.slice(i, i + CHUNK_SIZE));
    }

    // Summarize each chunk with Haiku (cheap + fast)
    const summaries = [];
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`\n✅  Claude Legacy Chat v6.5 running on http://localhost:${PORT}`);
